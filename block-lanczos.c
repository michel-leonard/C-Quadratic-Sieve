...
uint64_t *lanczos_block_worker(qs_sheet *qs) {
	...
	uint64_t *md[6], *xl[2], *sm[13], *tmp, *res, mask_0, mask_1, n_iterations = 0;
	qs_sm i, dim_0, dim_1;
	...
	} while (++n_iterations < 2048 && dim_0 && i != 64);
	...
	*res = 0; // mask
	...
	if (*res)
		DEBUG_NORMAL("Linear algebra completed after %d iterations.\n", n_iterations);
...
void lanczos_reduce_matrix(qs_sheet *qs) {
	...
	DEBUG_NORMAL("Linear algebra wants a matrix reduced from %u to %u.\n", qs->relations.length.prev, qs->relations.length.now);
}
