void print_help(const char *path) {
	...
	printf("  -r, --rand-seed <NUMBER>     Use another set of random numbers that depend on <NUMBER>, useful to generate test data.\n");
	printf("  -v, --verbose <VALUE>        The value can be set between 0 and 4 to displays more or less information.\n");
	...
	printf("  -j, --output-json            Format the results in JSON with pretty print.\n");
	printf("  -J, --output-json-compact    Format the results in JSON compact.\n");
	printf("  -c, --output-csv             Format the results in CSV.\n");
	printf("  -sql, --output-sql           Format the results in SQL.\n");
	printf("\n");
	printf("QUADRATIC SIEVE OPTIONS:\n");
	printf("  --qs-multiplier <value>      Usually from 1 to 150\n");
	printf("  --qs-base-size <value>       Usually from 1000 to 50000\n");
	printf("  --qs-large-prime <value>     Usually from 300000 to 30000000\n");
	printf("  --qs-alloc-mb <value>        Usually from 1 to a few hundred\n");
	printf("  --qs-sieve <value>           Usually close to a multiple of 31744\n");
	printf("  --qs-threshold <value>       Usually from 60 to 110\n");
	printf("  --qs-error-bits <value>      Usually from 15 to 35\n");
	printf("  --qs-laziness <value>        Usually from 80 to 120 (in percentage)\n\n");
	...
	printf("\n");
	printf("TESTING:\n");
	printf("  %s --generate -r 123 # provide a 'generated.txt' file that depends on the seed 123, suitable as input file.\n", path);
	...
	printf("  %s -i generated.txt -v 2   # Factor the testing file and display the success status and duration.\n", path);
	printf("\n");
	...
	printf("\n");
	printf("  This factorization software v2.0.0 was released by Michel Leonard on 1 February 2025.\n\n");
}
...
#define QS_PARAM(name_1, name_2) \
	...
	QS_PARAM(multiplier, multiplier) QS_PARAM(base - size, base_size) QS_PARAM(large - prime, large_prime) QS_PARAM(alloc-mb, alloc_mb) QS_PARAM(sieve, sieve) QS_PARAM(threshold, threshold) QS_PARAM(error - bits, error_bits) QS_PARAM(laziness, laziness)
	else
		return 0;
#undef QS_PARAM
	*argv = *(argv + 1) = 0;
...
void simple_rand(cint_sheet *sheet, uint64_t *seed, cint *nums, char *com, int n_factors, int bits) {
...
// Fill in the comment if the resulting number intentionally contains a power, empty it otherwise.
	...
	*com = 0;
	...
	int curr_bits = 0, i = 0, sym = 'A';
	cint_reinit(N, 1);
	do {
		const int factor_bits = (bits - curr_bits) / n_factors;
		retry :
		if (factor_bits < 3)
			goto begin;
		...
		*P->mem |= 1;
		if (!cint_is_prime(sheet, P, -1, seed))
			goto retry;
		i = !i;
		if (2 < n_factors && !xor_rand(seed, 0, 24)) {
			// Occasionally raise this prime number to a power.
			int pow = (int) xor_rand(seed, 2, n_factors - 1);
			n_factors += 1 - pow;
			com += sprintf(com, " %s %c^%d", sym == 'A' ? "and" : "*", sym, pow);
			++sym;
			do {
				cint_mul(N, P, A);
				if (--pow)
			...
			cint_mul(N, P, A);
		const int old_bits = curr_bits;
		curr_bits = (int) cint_count_bits(A);
		if (n_factors == 1 && curr_bits != bits) {
			curr_bits = old_bits;
			goto retry;
		}
		TMP = A, A = N, N = TMP;
...
void generate_input_file(state *state) {
	...
	if (fp) {
		...
		qs_md *g = state->params.generate, min = g[0], max = g[1], count = g[2], tmp, i;
...
void output(state *state) {
	...
	switch (state->params.output_format) {
		case 'J' :
			output_json_pretty_print(state, has_prev, has_next);
			break;
		case 'j' :
			output_json_compact(state, has_prev, has_next);
			break;
		case 'c' :
			output_csv(state, has_prev, has_next);
			break;
		case 's' :
			output_sql(state, has_prev, has_next);
			break;
		default :
			output_default(state, has_prev, has_next);
			break;
	}
	++state->scale.row_idx; // Update the index after each factorization.
	if (state->params.tty && 1 < state->params.verbose)
...
int factorization_prime_number_checker(state *state, int bits) {
	assert(64 < bits);
	cint_sheet *sheet = state->session.sheet;
	int is_prime = cint_is_prime(sheet, &state->session.num, -1, state->session.seed) != 0;
	if (is_prime) {
...
void factor(state *state) {
	...
	if (state->code == 0)
		for (int i = start_idx; state->session.res[i].power; ++i)
			...
			state->code |= !state->session.res[i].prime;
...
void process_multi(int argc, char **argv, state *state) {
	...
	if (1 < state->params.verbose)
		fprintf(stderr, "\nThe input %s fully factored in %.02f s.\n", state->code ? "[ was not ]" : "has been", (double) state->duration_ms * 0.001);
