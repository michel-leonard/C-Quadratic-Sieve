...
int factorization_quadratic_sieve(state * state, int bits) {
	// The state contain a number N from the factorization manager, and the Quadratic Sieve must :
	// - use resources present in the state (temporary variables + parameters)
	// - register prime (as possible) factors of N using the state
	// - update N accordingly to the registered factors
	// - return a non-zero value if a factor was registered

	if (bits < 65 || (DEFAULT_BIT_LIMIT_QS < bits && !state->params.force))
		return 0; // The execution time is doubled for each 10-bit increase.

	// On the testing machine : 260 bits take 12 minutes, 270-bit take 24 minutes, 310-bit take 5 hours.
...
void iteration_part_1(qs_sheet * qs, const cint * D, cint * A) {
	...
	if (qs->base.length <= i) {
		DEBUG_CRITICAL("Quadratic Sieve identified that A=%s is inconsistent.\n", simple_cint_string(qs->state, A));
		assert(++n_tries <= 9); // shouldn't happen 9 times, increase the factor base size otherwise.
		A = _A ;
		goto retry;
	}
...
#define IN_RANGE(F) (h_cint_compare(&qs->constants.ONE, F) < 0 && h_cint_compare(F, &qs->vars.N) < 0)
	...
	cint *curr, *Q = qs->vars.TEMP + 3, *R = Q + 1;
	int i = 0, pow;
	tasks[i].origin = 0, tasks[i++].num = tmp; // Stack-based recursion.
	...
	do {
		...
		if (cint_is_prime(qs->sheet, curr, -1, qs->seed)) {
			...
			assert(pow); // This prime factor is now removed from N.
			++qs->divisors.total_primes;
			qs->n_bits = (qs_sm) cint_count_bits(&qs->vars.N);
			// Register this prime factor to the manager's routine.
			manager_add_factor(qs->state, curr, pow, 1);
			if (tasks[i].origin) {
				char o = tasks[i].origin, *str = o == 1 ? "Allows the algorithm" : o == 2 ? "Prunes the divisors" : o == 3 ? "Divides N" : o == 4 ? "Notes a perfect power" : "Performs GCD within the divisors" ;
				DEBUG_MATH("%*s- %s to identify %s.\n", (i + 1) << 1, "", str, simple_cint_string(qs->state, curr));
			}
			if (1 < qs->n_bits) {
				DEBUG_MATH("%*s- This prime factor reduces N to %d-bit.\n", (i + 1) << 1, "", qs->n_bits);
				if ((tmp = qs_divisors_uniqueness_helper(qs, &qs->vars.N)))
					tasks[i].origin = 1, tasks[i++].num = tmp; // Allows the algorithm.
				for (qs_sm j = 0; j < qs->divisors.length; ++j) {
					cint_dup(F, qs->divisors.data[j]);
					pow = cint_remove(qs->sheet, F, curr);
					if (pow) {
						tmp = qs->divisors.data[j] = avl_at(&qs->uniqueness[2], F)->key;
						if (qs->uniqueness[2].affected && IN_RANGE(tmp))
							tasks[i].origin = 2, tasks[i++].num = tmp; // Prunes the divisors.
					}
				}
			} else {
				const char *str = tasks[i].origin ? "this" : simple_cint_string(qs->state, curr);
				DEBUG_MATH("%*s- Factorization completed since %s is a prime.\n", (i + 1) << 1, "", str);
			}
		} else {
			cint_div(qs->sheet, &qs->vars.N, curr, Q, R) ;
			if (R->mem == R->end && IN_RANGE(Q) && (tmp = qs_divisors_uniqueness_helper(qs, Q)))
				tasks[i].origin = 3, tasks[i++].num = tmp; // Divides N.
			pow = factorization_any_root_checker(qs->state, curr, Q, R) ;
			if (pow && IN_RANGE(Q) && (tmp = qs_divisors_uniqueness_helper(qs, Q)))
				tasks[i].origin = 4, tasks[i++].num = tmp; // Notes a perfect power.
			for (qs_sm j = 0; j < qs->divisors.length; ++j) {
				cint_gcd(qs->sheet, curr, qs->divisors.data[j], Q);
				if (IN_RANGE(Q) && (tmp = qs_divisors_uniqueness_helper(qs, Q)))
					tasks[i].origin = 5, tasks[i++].num = tmp ; // Performs GCD within the divisors.
			}
			qs->divisors.data[qs->divisors.length++] = curr;
		}
	} while (i && 1 < qs->n_bits);
	return 1 < qs->n_bits;
#undef IN_RANGE
...
void register_partial_relation(qs_sheet * qs, const cint * KEY, const cint * VALUE, const qs_sm * const restrict args[4]) {
	if (qs->n_bits < 160)
		return; // The process does not have time to materialize for small numbers.
...
void finalization_part_1(qs_sheet * qs, const uint64_t * restrict const lanczos_answer) {
	...
	for(qs_sm row = 0; row < 64 && 1 < qs->n_bits; ++row)
		if (mask >> row & 1){
...
int finalization_part_2(qs_sheet *qs) {
	if (qs->n_bits != 1 && qs->divisors.length) {
		// N is not fully factored, register divisor(s) of N from smallest to manager routine.
		...
		for (qs_sm i = 0; i < qs->divisors.length; ++i) {
			for (qs_sm j = 1 + i; j < qs->divisors.length; ++j)
				if (h_cint_compare(divisors[j], divisors[i]) < 0)
					tmp = divisors[i], divisors[i] = divisors[j], divisors[j] = tmp; // Partially apply a selection sort.
			if (h_cint_compare(&qs->constants.ONE, divisors[i]) < 0 && h_cint_compare(divisors[i], &qs->vars.N) < 0) {
				...
				if (power) {
					DEBUG_NORMAL("Quadratic Sieve identified the composite divisor %s.\n", simple_cint_string(qs->state, divisors[i]));
					manager_add_factor(qs->state, divisors[i], power, -1);
					qs->n_bits = (qs_sm) cint_count_bits(&qs->vars.N);
				}
			} else
				break; // No need to sort more.
		}
		// Starting another Quadratic Sieve with the remainder is faster than continuing this oversized instance.
		DEBUG_NORMAL("Quadratic Sieve reduced N to the %d-bit number %s.\n", qs->n_bits, simple_cint_string(qs->state, &qs->vars.N));
	}

	if (qs->uniqueness[1].count)
		DEBUG_NORMAL("Large-Prime Variation with %u partials added %u smooth relations.\n", (unsigned) qs->uniqueness[1].count, qs->relations.length.by_partial);
	DEBUG_NORMAL("The efficiency ratio is %.02f with %u polynomials and %u relations.\n", (double) qs->relations.length.peak / (double) qs->poly.curves, qs->poly.curves, qs->relations.length.peak);
