...
- is capable of batch factoring numbers with JSON or CSV output.
- takes its I/O from the command line or from files.
...
Otherwise, to build the executable, Windows users should use **PowerShell**, while macOS and Linux users should use the **Terminal**. The procedure takes just a few minutes :
...
For every additional 10 bits (or 3 decimal digits) to the input number, the factorization duration roughly doubles.
...
The software has been extensively tested during its preparation and has a **production ready** status. The built-in generator, consistent across all platforms, produces reproducible samples in a `demand.txt` file. This feature is designed to quickly simulate factorization demands.
...
- update the variable `OEIS` to target another list such as [Hyperfactorials](https://oeis.org/A002109/b002109.txt) list at `002109` in the [OEIS](https://oeis.org/) model.
...
An exit code of `0` is interpreted as `The results are completely accurate and verified` and means that :
- the product of the factors is equal to the factored number, with verification performed for each number.
- all numbers have been fully factored using a [good confiance level](big-num.c#L725) when testing for primality.
